...
#Define Wavelet Transform Function - Single Level

def waveletTransformEightLevels(df):
  ...
  for i in range (len(df)):
    aux = df.loc[i]
    cA8, cD8, cD7, cD6, cD5, cD4, cD3, cD2, cD1= pywt.wavedec(aux,'db2', level = 8) 
    cA8 = pd.DataFrame(cA8).T
    cD8 = pd.DataFrame(cD8).T
    cD7 = pd.DataFrame(cD7).T
    cD6 = pd.DataFrame(cD6).T
    cD5 = pd.DataFrame(cD5).T
    cD4 = pd.DataFrame(cD4).T
    cD3 = pd.DataFrame(cD3).T
    cD2 = pd.DataFrame(cD2).T
    cD1 = pd.DataFrame(cD1).T
    aux_df2 = pd.concat([cA8,cD8,cD7,cD6,cD5,cD4,cD3,cD2,cD1], axis=1)
    aux_df = pd.concat([aux_df,aux_df2])

  c = list(range(0, 3019, 1))
...
x = waveletTransformEightLevels(x1)
x = pd.DataFrame(x)
